...
| ลบบัญชีตัวเอง (ข้อมูลทั้งหมดหายถาวร) | | ✓ | ✓* |
| กดหัวใจ · แจ้งปัญหา | | ✓ | ✓ |
| ซ่อน / แก้ / ลบชีทของทุกคน | | | ✓ |
| ดูและปิดรายงานปัญหา | | | ✓ |
| ตั้ง/ถอดสิทธิ์แอดมิน · ระงับผู้ใช้ | | | ✓ |

* แอดมินคนสุดท้ายลบบัญชีไม่ได้ ต้องตั้งแอดมินคนอื่นก่อน

**กันสแปม:** แบ่งปันได้ 10 ชีท และอัปโหลดไฟล์ได้ 15 ไฟล์ ต่อคนต่อ 24 ชม. (แอดมินไม่จำกัด) · บัญชีที่ถูกระงับแก้/ลบชีทไม่ได้

**ลบบัญชี** (`/me` → ลบบัญชี): ลบไฟล์ชีททั้งหมดใน storage แล้วลบผู้ใช้ออกจาก Supabase Auth — โปรไฟล์ ชีท หัวใจ และรายงาน ถูกลบตามด้วย `on delete cascade` (ยอดหัวใจของชีทคนอื่นลดลงอัตโนมัติ)

**หน้านโยบาย:** `/privacy` (นโยบายความเป็นส่วนตัวตาม PDPA) และ `/terms` (กติกาการใช้งาน) — เป็นร่างที่ควรให้ครูหรือสภานักเรียนตรวจก่อนเปิดใช้จริง ตั้งช่องทางติดต่อผ่าน `NEXT_PUBLIC_CONTACT`
...
- PDF/รูป ถูกวาดลง `<canvas>` ด้วย pdf.js (ไม่มีปุ่มดาวน์โหลดของ PDF viewer) พร้อมลายน้ำอีเมลผู้อ่าน — วาดเฉพาะหน้าที่ใกล้จอ หน้าที่เลื่อนผ่านไปไกลจะคืนหน่วยความจำ (PDF ยาว ๆ ไม่ทำให้มือถือค้าง)
...
app/
  ...
  me/                 ชีทของฉัน + ชื่อที่แสดง + ลบบัญชี
  privacy/ terms/     นโยบายความเป็นส่วนตัว / กติกาการใช้งาน
...
- ช่องทางติดต่อผู้ดูแล (แสดงในหน้านโยบาย): `NEXT_PUBLIC_CONTACT`
- จำนวนชีท/ไฟล์ต่อวัน: `limit_sheet_uploads()` และ `recent_upload_count()` ใน `supabase/schema.sql`
